...
// In connect.js, modify your chessboard initialization:
var board = Chessboard('board', {
    position: 'start',
    draggable: true,
    pieceTheme: 'https://chessboardjs.com/img/chesspieces/wikipedia/{piece}.png', // Add this line
    onDragStart: onDragStart,
    onDrop: onDrop,
    onSnapEnd: onSnapEnd
});

// Add this to your connect.js, in the drawArrow function
function drawArrow(from, to) {
    ...
    if (!fromSquare || !toSquare) {
        console.error('Square not found:', from, to);
        return;
    }
    ...
    // Create arrow line (make it thicker for better visibility)
    const line = document.createElement('div');
    line.className = 'arrow-line';
    line.style.width = (length - 16) + 'px'; // Subtract for arrowhead
    line.style.height = '8px'; // Thicker line
    line.style.transform = `rotate(${angle}deg)`;
    line.style.backgroundColor = 'rgba(255, 0, 0, 0.8)'; // More opaque
    arrow.appendChild(line);
    
    // Create arrow head (make it bigger)
    const head = document.createElement('div');
    head.className = 'arrow-head';
    head.style.transform = `translate(${length - 16}px, -12px) rotate(${angle}deg)`;
    head.style.borderWidth = '12px 0 12px 20px'; // Bigger arrow head
    head.style.borderColor = 'transparent transparent transparent rgba(255, 0, 0, 0.8)';
